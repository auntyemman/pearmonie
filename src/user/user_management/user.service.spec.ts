...
describe('UserService', () => {
  ...
  describe('getUser', () => {
    it('should throw an error if user is not found', async () => {
      ...
      await expect(userService.getUser(3)).rejects.toThrow(NotFoundError);

      expect(userRepositoryMock.findById).toHaveBeenCalledWith(3);
    });
  ...
  describe('updateUser', () => {
    it('should throw an error if update fails', async () => {
      ...
      await expect(userService.updateUser(3, { email: 'test@test.com' })).rejects.toThrow(APIError);

      expect(userRepositoryMock.update).toHaveBeenCalledWith(3, { email: 'test@test.com' });
    });

    it('should return the updated user', async () => {
      userRepositoryMock.update.mockResolvedValue({ email: 'test@test.com' } as IUser);

      const result = await userService.updateUser(3, { email: 'test@test.com' });
